...
/*					nEl.addClass('editable');
			...
			// Checks if there is an actual match between two teams
			...
			// Gets team names in match from click event
			var teamNames = function(e) {
				var arr = [];
				for (var i = 0; i < 2; i++) {
					arr.push($(e.target.parentElement.parentElement).find('.label')[i].innerHTML);
				}
				return arr;
			};
			// Remove condition where must not be editable and clickable at same time
			//if (!opts.save) {
				...
				if (opts.onMatchClick)
					teamCon.click(function (e) {
						// Make sure click on a valid match
						if (validMatch(e)) {
							//console.log('click', e.target, teamNames(e));
							opts.onMatchClick(teamNames(e)); // onMatchClick to be called on label lick
						}
					});
			//}
			match.a.id = 0;
	...
	var methods = {
		init: function (opts) {
			...
			opts.init.teams = !opts.init.teams || opts.init.teams.length == 0 ? [
				["", ""]
			] : opts.init.teams;
		...
		render: function () {
			var bracket = $(this).data('bracket');
			return bracket.obj.render(false);
		}
	};
